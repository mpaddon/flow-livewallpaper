...
public class LiveWallpaperSettings extends PreferenceActivity implements
	...
	protected void onCreate(Bundle icicle) {
		...
		_themePreference = (PhotoBarPreference) getPreferenceScreen()
				.findPreference("theme_preference");
		_photoPreference = (PhotoBarPreference) getPreferenceScreen()
				.findPreference("background_photo_preference");
		_waterPreference = (AmbilWarnaPreference) getPreferenceScreen()
				.findPreference("water_color_preference");
		_sunPreference = (AmbilWarnaPreference) getPreferenceScreen()
				.findPreference("sun_color_preference");
		_advancedSettingsPreference = (TogglePreference) getPreferenceScreen()
				.findPreference("advanced_preference_title");
		_advancedSettingsCategory = (PreferenceCategory) getPreferenceScreen()
				.findPreference("advanced_preference_key");

		// seekbars
		_flowSpeedPref = (SeekBarPreference) getPreferenceScreen()
				.findPreference("flow_speed_preference");
		_flowDistPref = (SeekBarPreference) getPreferenceScreen()
				.findPreference("flow_distortion_preference");
		_flowWavePref = (SeekBarPreference) getPreferenceScreen()
				.findPreference("flow_wave_preference");
		;
		_touchDistPref = (SeekBarPreference) getPreferenceScreen()
				.findPreference("input_dis_preference");
		;
		_touchStrokePref = (SeekBarPreference) getPreferenceScreen()
				.findPreference("brushstroke_preference");
		;

		_expanded = _advancedSettingsPreference.expanded;
		if (!_expanded) {
		...
		_waterPreference.forceSetValue(_sp.getInt("water_color_preference",
				FlowTheme._themeTable.get((long) 7).WaterColor));
		_sunPreference.forceSetValue(_sp.getInt("sun_color_preference",
				FlowTheme._themeTable.get((long) 7).SunColor));
	...
	public void onSharedPreferenceChanged(SharedPreferences sharedPreferences,
		...
		if (key.equals("water_color_preference")) {
			...
			int val = (int) (sharedPreferences.getFloat(key, .5f) * SeekBarPreference.MAX_VALUE);
			_flowWavePref.forceSetValue(val);
